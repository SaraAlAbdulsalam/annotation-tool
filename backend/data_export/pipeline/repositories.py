...
## ~~~~~~ if the offset not cover whole word, I change to complete it
    def label_per_user(self, doc) -> Dict:
        label_per_user = defaultdict(list)
        for a in doc.spans.all():
            First = a.start_offset
            Second = a.end_offset
            word = doc.text

            while ( First != 0 and word[First-1] != " "):
                First = First - 1
            while (word[Second] != " " and Second != len(word)-1 ):
                Second = Second + 1
            if word[Second] ==" ":
                whole_word = word[First:Second]
            else:
                whole_word =  word[First:Second+1]
            label = (a.start_offset, a.end_offset, First, Second, a.label.text,whole_word)
